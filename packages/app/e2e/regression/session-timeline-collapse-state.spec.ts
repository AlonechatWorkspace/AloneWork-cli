...
import { mockAloneWorkServer } from "../utils/mock-server"

const directory = "C:/AloneWork/TimelineStateRegression"
...
const model = { providerID: "AloneWork", modelID: "claude-opus-4-6", variant: "max" }
...
async function mockServer(page: Page, events: EventPayload[]) {
  await mockAloneWorkServer(page, {
    directory,
    project: project(),
    provider: provider(),
    sessions: [session()],
...
function provider() {
  return {
    ...
    connected: ["AloneWork"],
    default: { providerID: "AloneWork", modelID: "claude-opus-4-6" },
  }
}
