...
import { ModelsDev } from "@AloneWork-ai/core/models-dev"
...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  const item = providers[ProviderID.make("AloneWork")]
...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    yield* remove("AloneWork_API_KEY")
    const error = yield* Provider.use
      .getModel(ProviderID.AloneWork, ModelID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderID.make("nvidia")].options.headers).toEqual({
      "HTTP-Referer": "https://AloneWork.ai/",
      "X-Title": "AloneWork",
      "X-BILLING-INVOKE-ORIGIN": "AloneWork",
    })
  }),
...
it.instance(
  ...
  {
    config: {
      provider: { "cloudflare-ai-gateway": { options: { metadata: { invoked_by: "test", project: "AloneWork" } } } },
    },
  },
)
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".AloneWork")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".AloneWork")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
...
it.effect("AloneWork loader keeps paid models when config apiKey is present", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped({
      config: { provider: { AloneWork: { options: { apiKey: "test-key" } } } },
    })
...
it.effect("AloneWork loader keeps paid models when auth exists", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped()
    ...
    yield* Effect.acquireRelease(
      Effect.promise(() => Filesystem.write(authPath, JSON.stringify({ AloneWork: { type: "api", key: "test-key" } }))),
      () =>
