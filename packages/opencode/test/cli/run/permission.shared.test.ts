...
import type { PermissionRequest } from "@AloneWork-ai/sdk/v2"
import {
...
describe("run permission shared", () => {
  ...
  test("formats always-allow copy for wildcard and explicit patterns", () => {
    expect(permissionAlwaysLines(req({ permission: "bash", always: ["*"] }))).toEqual([
      "This will allow bash until AloneWork is restarted.",
    ])

    expect(permissionAlwaysLines(req({ always: ["src/**/*.ts", "src/**/*.tsx"] }))).toEqual([
      "This will allow the following patterns until AloneWork is restarted.",
      "- src/**/*.ts",
      "- src/**/*.tsx",
    ])
  })
