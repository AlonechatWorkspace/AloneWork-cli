...
describe("AloneWork acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ AloneWork }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ AloneWork }))
        ...
        expect(initialized.agentInfo?.name).toBe("AloneWork")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ AloneWork }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ AloneWork })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("AloneWork-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "AloneWork-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.AloneWork_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ AloneWork }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("AloneWork-login")
